...
// Se vier algo fora da whitelist, cai no padrão 'nome'
...
// -------------------- Aplica filtro de busca em PHP --------------------
$dados = [];
if ($result) {
    $tem_busca = ($buscar !== '');
    if ($tem_busca) {
        // Termos para comparação (case-insensitive e sem acento)
        $busca_normalizada = normalizar($buscar);
        $busca_numerica    = preg_replace('/[^0-9]/', '', $buscar);
    }

    while ($row = $result->fetch_assoc()) {
        if ($tem_busca) {
            // --- Nome (sempre correspondência parcial, ignorando maiúsc./minúsc. e acentos) ---
            $nome_norm = normalizar((string)$row['nome']);
            $ok_nome   = (strpos($nome_norm, $busca_normalizada) !== false);

            // --- Endereço formatado ("endereco, numero" quando houver) - também parcial/insensível ---
            ...
            // --- Telefone (só dígitos; mantém igualdade exata) ---
            $tel_digits  = preg_replace('/[^0-9]/', '', (string)$row['telefone']);
            $ok_telefone = ($busca_numerica !== '' && $tel_digits === $busca_numerica);

            // --- Data/Hora (mantém igualdade exata ao valor bruto do banco) ---
            $ok_data = ((string)$row['data_hora'] === $buscar);

            // Se nada casar, descarta a linha
            if (!($ok_nome || $ok_endereco || $ok_telefone || $ok_data)) {
                continue;
            }
        }
...
function formatEndereco(array $row): string {
    ...
    // considera NULL vindo como string e ignora vazio
    if ($n !== '' && strtolower($n) !== 'null') {
        // evita vírgula duplicada no fim do endereço
        $e = preg_replace('/\s*,\s*$/', '', $e);
        // se quiser evitar repetir o número caso já esteja no texto:
        if (stripos($e, $n) === false) {
            $e .= ($e !== '' ? ', ' : '') . $n;
        }
    }
    return $e;
...
function render_rows($dados) {
    foreach ($dados as $row) { ?>
        <tr id="row-<?= (int)$row['id'] ?>">
            ...
            <td><?= badgeStatus($row['status_reclamacao'] ?? 'Pendente') ?></td>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="visualizar_reclamacoes.php"><i class="bi bi-eye"></i> Ver Requisição</a></li>
...
<h2 class="text-center mb-4">Reclamações</h2>
...
<form method="GET" class="row g-3 mb-3">
    ...
    <div class="col-auto">
        <input type="text" name="buscar" value="<?= htmlspecialchars($buscar) ?>" class="form-control" placeholder="Buscar por nome, telefone ou data">
    </div>
    <div class="col-auto d-flex align-items-center">
        <input type="checkbox" name="incluir" id="incluir" class="form-check-input me-1" <?= $incluir?'checked':'' ?>>
        <label for="incluir" class="form-check-label">Incluir</label>
    </div>
    <div class="col-auto">
...
<!-- SELECIONAR FAIXA + AÇÕES -->
<div class="row g-3 mb-3 align-items-end">
  <!-- Seleção por Ordem -->
  <div class="col-auto">
    <input type="number" id="ordemInicio" class="form-control" placeholder="De Ordem">
  </div>
  <div class="col-auto">
    <input type="number" id="ordemFim" class="form-control" placeholder="Até Ordem">
  </div>
  <div class="col-auto">
    <button type="button" class="btn btn-primary" id="btnSelecionarOrdem">Selecionar por Ordem</button>
  </div>
  ...
  <div class="col-auto">
    <div class="btn-group">
      <button class="btn btn-danger dropdown-toggle" data-bs-toggle="dropdown" type="button">
        <i class="bi bi-file-earmark-pdf"></i> PDF
      </button>
      <ul class="dropdown-menu dropdown-menu-start shadow">
        <li>
          <a class="dropdown-item" href="#" id="btn-pdf-selecionados">
            <i class="bi bi-check2-square me-2"></i>Selecionados
          </a>
        </li>
        <li>
          <a class="dropdown-item" href="gerar_pdf.php?tipo=20recentes" target="_blank">
            <i class="bi bi-clock-history me-2"></i>21 mais recentes
          </a>
        </li>
        <li>
          <a class="dropdown-item" href="#" id="btn-pdf-id">
            <i class="bi bi-hash me-2"></i>Por ID…
          </a>
        </li>
      </ul>
    </div>
  ...
  <div class="col-auto">
    <div class="btn-group">
      ...
      <ul class="dropdown-menu dropdown-menu-start shadow">
        <li>
          <a class="dropdown-item" href="exportar_excel.php" target="_blank">
            <i class="bi bi-download me-2"></i>Exportar tudo
          </a>
        </li>
        <li>
          <a class="dropdown-item" href="exportar_excel_10_recentes.php" target="_blank">
            <i class="bi bi-lightning me-2"></i>10 mais recentes
          </a>
        </li>
        <li>
          <a class="dropdown-item" href="#" id="btn-excel-selecionados">
            <i class="bi bi-check2-square me-2"></i>Selecionados
          </a>
        </li>
        <li>
          <a class="dropdown-item" href="planilha_por_proximidade.php" target="_blank" id="btn-excel-proximidade">
            <i class="bi bi-geo me-2"></i>Por proximidade
          </a>
        </li>
      </ul>
    </div>
...
document.getElementById('btnSelecionarIntervalo')?.addEventListener('click', () => {
    ...
    if(isNaN(inicio)||isNaN(fim)){alert("Preencha os dois campos de ID."); return;}
    if(inicio>fim)[inicio,fim]=[fim,inicio];
    selecionados=[];
    for(let i=inicio;i<=fim;i++) selecionados.push(i.toString());
    localStorage.setItem('reclamacoesSelecionadas', JSON.stringify(selecionados));
    document.querySelectorAll('.chk-reclamacao').forEach(chk => chk.checked=selecionados.includes(chk.value));
});

// EXPORTAR EXCEL SELECIONADOS
document.getElementById('btn-excel-selecionados')?.addEventListener('click', (e) => {
    e.preventDefault();
    if(selecionados.length===0){alert("Selecione ao menos uma reclamação."); return;}
    window.open('exportar_excel.php?ids=' + selecionados.join(','),'_blank');
});

// EXPORTAR EXCEL POR PROXIMIDADE
document.getElementById('btn-excel-proximidade')?.addEventListener('click', (e) => {
    e.preventDefault();
    window.open('planilha_por_proximidade.php', '_blank');
});

// Seleção por ordem (posição na tabela)
document.getElementById('btnSelecionarOrdem')?.addEventListener('click', () => {
    let inicio = parseInt(document.getElementById('ordemInicio').value);
    let fim = parseInt(document.getElementById('ordemFim').value);
    let checkboxes = Array.from(document.querySelectorAll('.chk-reclamacao'));

    if(isNaN(inicio) || isNaN(fim)) {
        alert("Preencha os dois campos de Ordem.");
        return;
    }
    ...
    checkboxes.forEach((chk, index) => {
        let posicao = index + 1; // posição começa em 1
...
(() => {
  ...
  let pagina      = <?= (int)$pagina ?>;                   // página atual já renderizada
  ...
  async function carregarMais() {
    ...
    try {
      ...
      if (data && data.success) {
        ...
        // Rebind nos novos botões deletar
  ...
  function ligarObserver() {
    observer.disconnect();
    const alvo = document.getElementById('sentinela'); // posição correta atual
    if (alvo && temMais) observer.observe(alvo);
  }

  // (re)liga quando necessário
  ...
  // se o layout mudar (ex: mover botões), garantimos que o alvo seja reobservado
